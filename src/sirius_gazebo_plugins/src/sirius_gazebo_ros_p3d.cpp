...
namespace gazebo
{
GZ_REGISTER_MODEL_PLUGIN(SiriusGazeboRosP3D);

////////////////////////////////////////////////////////////////////////////////
// Constructor
SiriusGazeboRosP3D::SiriusGazeboRosP3D()
{
}
...
////////////////////////////////////////////////////////////////////////////////
// Load the controller
void SiriusGazeboRosP3D::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  // Get the world name.
  ...
  if (!_sdf->HasElement("frameXyzOffset"))
  {
    ROS_DEBUG_NAMED("p3d", "p3d plugin missing <frameXyzOffset>, defaults to 0s");
    this->frame_offset_.Pos() = ignition::math::Vector3d(0, 0, 0);
  }
  else
    this->frame_offset_.Pos() = _sdf->GetElement("frameXyzOffset")->Get<ignition::math::Vector3d>();

  if (!_sdf->HasElement("frameRpyOffset"))
  {
    ROS_DEBUG_NAMED("p3d", "p3d plugin missing <frameRpyOffset>, defaults to 0s");
...
void SiriusGazeboRosP3D::UpdateChild()
{
#ifdef ENABLE_PROFILER
  IGN_PROFILE("SiriusGazeboRosP3D::UpdateChild");
#endif
  if (!this->link_)
    return;
...
double SiriusGazeboRosP3D::GaussianKernel(double mu, double sigma)
{
...
void SiriusGazeboRosP3D::P3DQueueThread()
{
